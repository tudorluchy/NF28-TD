...
//			System.out.println("Pas de noeud Contact sélectionné.");
	...
	/**
	 * Load model and update tree and xml tab after xml file drop
	 * @param xmlFilePath
	 * @return
	 */
	public boolean loadModelFromFileDrop(String xmlFilePath) {
		// test xml file
		...
		try {
			// load model
			parse(xmlFilePath);
			// fire notifications to Tree and Xml view
			fireNotificationUpdateTreeAndXmlTab();
		} catch (Exception e) {
			e.printStackTrace();
			return false;
		}
	...
	/**
	 * Update xml tab after tree model change
	 */
	public void updateXmlAfterContactDrop() {
		fireNotificationUpdateXmlTab(model.toXml());	
	}
}
