...
import model.NodeTransfertHandler;
...
public class TreeView extends JFrame implements PropertyChangeListener {
	...
	public TreeView() {
		...
		// tree
		NodeTransfertHandler nth = new NodeTransfertHandler();
		treePanel = new ContactTreePanel(null);
		treePanel.setDragEnabled(true);
		treePanel.setTransferHandler(nth);
	...
	public void propertyChange(PropertyChangeEvent pce) {
		if (pce.getPropertyName().equals(ApplicationModel.PROPERTY_UPDATE_VIEW_CONTACT_TAB)) {
			...
			// go on tab xml
			tabbedPane.setSelectedIndex(0);
		} else if (pce.getPropertyName().equals(ApplicationModel.PROPERTY_UPDATE_VIEW_TREE_AND_XML_TAB)) {
    		// set tree model
    		treePanel.setModel(appModel.getModel());
    		// set xml text
    		xmlPanel.setTextArea(appModel.getModel().toXml());
